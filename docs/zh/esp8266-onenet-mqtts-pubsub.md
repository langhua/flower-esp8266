...
本节是说明如何把温度、湿度测量数据发布到OneNET MQTTS中，下面的代码结合了[ESP8266测量和显示温度/湿度](esp8266-dht11-lcd1602.md)中温度、湿度测量，以及[ESP8266连接OneNET MQTTS](esp8266-onenet-mqtts-connect.md)中的连接OneNET MQTTS：
...
代码第249-301行，是每10秒，测量一次温度和湿度，并发布到OneNET上：
...
运行上述代码，在串口监视器中，输出内容如下图所示：

![onenet mqtts pubsub](images/onenet/onenet_mqtts_pubsub.png)

<br/>

修改第466行的湿度数据格式，在湿度数字后面加个%，令OneNET解析时出错：

```c++
     << "}],'humidity':[{'v':" << humidity << "%,'t':" << now 
```

运行代码，在串口监视器中，会出现OneNET MQTTS返回的Illegal data错误，如下图所示：

![onenet mqtts pubsub illegaldata](images/onenet/onenet_mqtts_pubsub_illegaldata.png)

<br/>

把代码恢复之前的正确发送后，重新运行。访问OneNET MQTT套件的后台，可以按图表形式看到记录的数据，如下图所示：

![onenet mqtts data view](images/onenet/onenet_mqtts_data_view.png)
...
4. OneNET MQTT物联网套件-数据点 topic 簇：[https://open.iot.10086.cn/doc/mqtt/book/device-develop/topics/dp-topics.html](https://open.iot.10086.cn/doc/mqtt/book/device-develop/topics/dp-topics.html)